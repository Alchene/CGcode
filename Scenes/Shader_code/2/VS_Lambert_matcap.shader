    ...
    SubShader {
        ...
        Pass {
           ...
           //令Input Vertrx Position 输入结构体
            ...
            float3 nDirVS = mul(UNITY_MATRIX_V, float4(nDirWS,0.0));        //世界空间转换到视觉空间
            // i.nDirWS = normalize(i.nDirWS);                //归一化，不然像素
			// float3 nDirWS = i.nDirWS;
			float3 lDir = _WorldSpaceLightPos0.xyz;           //灯光方向
			float3 vDir = normalize(_WorldSpaceCameraPos.xyz - i.posWS.xyz);   //世界空间下
			float3 vrDir = reflect( -vDir, nDirWS);   //世界空间下
            //float3 nodtv =  dot(nDirWS，vdir);                    
            //中间量
            float2 matcapUV = nDirVS.rg * 0.5+0.5;
            //光照模型
            float3 matcap1 = tex2D(_Matcap,matcapUV); //matcap采样
			//Lambert+phong
            ...
            float3 envSpecLight = matcap1*Fresnal*_envSpecInt;
