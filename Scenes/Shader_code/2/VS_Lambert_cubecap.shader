Shader "VS/09cube"
{
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform samplerCUBE _Cubemap;                       //CUBEmap
           ...
           //令Input Vertrx Position 输入结构体
			...
			float3 vDir = normalize(_WorldSpaceCameraPos.xyz - i.posWS.xyz);   //世界空间下的法方向
			float3 vrDir = reflect( -vDir, nDirWS);   //观察空间下的反射方向
            //float3 nodtv =  dot(nDirWS，vdir);                    
            //中间量

            //光照模型
            float3 cubemap1 = texCUBElod(_Cubemap,float4(vrDir,_CubemapMip)); //ccubemap采样
            float4 AO_TEX_var = tex2D(_AO_TEX,TRANSFORM_TEX(i.uv0, _AO_TEX)); //AO采样
			//Lambert+phong
            ...
            float  Fresnal  =pow((1.0-max(dot(vrDir,nDirWS),0.0)),_Fresnal);
            float3 envSpecLight = cubemap1*Fresnal*nDirWS.g*_envSpecInt*AO_TEX_var.b*AO_TEX_var.g;

			float LPhong =hlafLambert + Phong;
            float3 LP = (LPhong,LPhong,LPhong) * BaseMap_var*_Diff + SSS_var.rgb + envSpecLight;
            //三色AO
			...
			float3 final = AO1 *LP;
            //float3 finalCol = (envSpecLight,envSpecLight,envSpecLight );
